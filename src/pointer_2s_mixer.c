...
static int process_and_report(const struct device *dev) {
    ...
    if (have_x || have_y) {
        const int32_t steady_thres = CONFIG_POINTER_2S_MIXER_STEADY_THRES;
...
static float calculate_twist(const struct device *dev) {
    ...
    if (!data->ema_initialized) {
        ...
        const float alpha = (float) CONFIG_POINTER_2S_MIXER_EMA_ALPHA / 100.0f;
    ...
    if (data->last_sig_move - now < (uint32_t)CONFIG_POINTER_2S_MIXER_STEADY_COOLDOWN) {
...
static void p2sm_save_work_cb(struct k_work *work) {
    ...
    int err = settings_save_one(key, &data->twist_reversed, sizeof(data->twist_reversed));
    if (err < 0) {
